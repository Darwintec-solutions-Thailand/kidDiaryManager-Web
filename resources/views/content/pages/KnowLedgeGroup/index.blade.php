...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-datatable table-responsive">
            ...
            <table class="tb_KnowLedgeGroup table">
                <thead>
                    <tr>
                        <th></th>
                        <th>ลำดับ</th>
    ...
    {{-- modalStoreData --}}
    @include('content.pages.KnowLedgeGroup.modalStoreData')

    {{-- modalPreView --}}
    @include('content.pages.KnowLedgeGroup.modalPreView')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btnSave').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('KnowLedgeGroup.store') }}",
                    data: {
                        '_token': "{{ csrf_token() }}",
                        'modalType': modalType,
                        'title': title,
            ...
            // ปุ่ม Preview ข้อมูล
            $('.tb_KnowLedgeGroup').on('click', '.previewData', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('KnowLedgeGroup.getDataEdit') }}",
                    data: {
                        '_token': "{{ csrf_token() }}",
                        'id': id,
                    },
                    success: function(result) {
                        ...
                        var html = '';
                        html += '<p class="mr-3">อายุขั้นสุด ' + result.age_min + '</p>'
                        html += '<p class="mr-3">อายุสูงสุด ' + result.age_max + '</p>'
            ...
            $('.tb_KnowLedgeGroup').on('click', '.btnDel', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('KnowLedgeGroup.destroy') }}",
                            data: {
                                '_token': "{{ csrf_token() }}",
                                'id': id,
                            },
            ...
            //ปุ่มแก้ไข
            $('.tb_KnowLedgeGroup').on('click', '.btnEdit', function() {
                var id = $(this).attr("data-id");
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        $('#ageMin').val(result.age_min)
                        $('#ageMax').val(result.age_max)
                    },
                    error: function(xhr, status, error) {
                        console.log("getDataJSTree : xhr", xhr.responseText);
                        console.log(xhr.responseJSON.exception)
            ...
            // ข้อมูลของตาราง
            var table = $('.tb_KnowLedgeGroup').DataTable({
                ajax: {
                    "url": "{{ route('KnowLedgeGroup.getData') }}",
                    "type": "POST",
                    "data": {
                        '_token': _token
                    },
